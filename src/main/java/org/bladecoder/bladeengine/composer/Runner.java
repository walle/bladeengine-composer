...
import org.bladecoder.bladeengine.composer.utils.DinamicClassPath;
...
public class Runner extends BladeEngine {
  ...
  Runner() {
    ...
    try {
      InputStream s = Runner.class.getResourceAsStream("/" + Config.PROPERTIES_FILENAME);
      if(s!=null)
        p.load(s);
    } catch (IOException e) {
    }
...
//		cfg.useGL30 = true;
  ...
  public void run() {
    if(Runner.class.getResource("/icons/icon128.png")!=null)
      cfg.addIcon("icons/icon128.png", FileType.Internal);

    if(Runner.class.getResource("/icons/icon32.png")!=null)
      cfg.addIcon("icons/icon32.png", FileType.Internal);

    if(Runner.class.getResource("/icons/icon16.png")!=null)
